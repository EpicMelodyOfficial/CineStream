        ...
        {filteredSeasons.map((season: any) => (
          ...
          >
            <div className="aspect-[2/3] w-full relative bg-black/50">
              {season.poster_path ? (
                <Image src={getImageUrl(season.poster_path, 'w500')} alt={season.name} fill className="object-cover" />
              ) : (
            ...
            <div className="p-4">
              ...
              <div className="text-[12px] text-white/60 space-y-0.5">
                ...
                {season.air_date && <p>Released {new Date(season.air_date).getFullYear()}</p>}
              </div>
            </div>
          </div>
        ))}
            ...
            <div>
              <div className="flex flex-col md:flex-row gap-8 mb-10">
                {seasonDetails.poster_path && (
                  <div className="w-32 md:w-48 shrink-0 rounded-xl overflow-hidden ring-1 ring-white/10 hidden md:block">
                    <Image src={getImageUrl(seasonDetails.poster_path, 'w500')} alt={seasonDetails.name} width={200} height={300} className="w-full h-auto object-cover" />
                  </div>
                )}
                <div>
                  ...
                  <div className="flex items-center gap-4 text-[14px] font-medium text-white/60 mb-6">
                    {seasonDetails.air_date && (
                      <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4" /> {new Date(seasonDetails.air_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                    )}
              ...
              <div className="space-y-4">
                ...
                {seasonDetails.episodes?.map((episode: any) => (
                  <div key={episode.id} className="flex flex-col sm:flex-row gap-4 sm:gap-6 p-4 rounded-xl bg-black/40 hover:bg-white/5 border border-white/5 transition-colors group">
                    <div className="w-full sm:w-48 aspect-video shrink-0 rounded-lg overflow-hidden relative bg-white/5">
                      {episode.still_path ? (
                        <Image src={getImageUrl(episode.still_path, 'w500')} alt={episode.name} fill className="object-cover" />
                      ) : (
                    ...
                    <div className="flex-1 py-1">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-2">
                        ...
                        <div className="flex items-center gap-3 text-[13px] font-medium text-white/50 shrink-0">
                          {episode.air_date && <span>{new Date(episode.air_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
